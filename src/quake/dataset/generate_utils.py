...
class Geometry:
    ...
    def __init__(self, detector: dict):
        ...
        if detector["should_crop_planes"]:
            ...
            self.nb_xbins_no_crop = self.nb_xbins 
            self.nb_ybins_no_crop = self.nb_ybins
            self.nb_zbins_no_crop = self.nb_zbins
            self.nb_xbins = math.ceil((xmax_reduced - xmin_reduced) / self.xbin_w)
            self.nb_ybins = math.ceil((ymax_reduced - ymin_reduced) / self.ybin_w)
            self.nb_zbins = math.ceil((zmax_reduced - zmin_reduced) / self.zbin_w)
