...
def tr_val_te_split(data, labels, opts, seed):
    ...
    s_tv, s_te, l_tv, l_te = train_test_split(
        data_idx, labels, test_size=test_size, random_state=seed
    )
    s_tr, s_val, l_tr, l_val = train_test_split(
        s_tv, l_tv, test_size=val_size, random_state=seed
    )
