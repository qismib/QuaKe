...
def CNN_train(data_folder: Path, setup):
    ...
    logger.info(
        f"{len(l_tr)} samples for train ("
        f"{len(l_tr)/ntot:.1%} of total) ("
        f"{frac(l_tr, 1):.1%} signal, "
        f"{frac(l_tr, 0):.1%} background)"
    )
    logger.info(
    ...
    model.fit(
        [s_tr[0], s_tr[1], s_tr[2]],
        to_categorical(l_tr),
        batch_size=batch_size,
        epochs=epochs,
        validation_data=([s_val[0], s_val[1], s_val[2]], to_categorical(l_val)),
        verbose=1,
    )

    scores = model.predict([s_te[0], s_te[1], s_te[2]])
    y_prob = scores[:, 1]
