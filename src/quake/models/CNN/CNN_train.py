...
sensitivity = lambda y, l: np.sum(1 * np.logical_and(y == 1, l == 1)) / np.sum(
    1 * l == 1
)
specificity = lambda y, l: np.sum(1 * np.logical_and(y == 0, l == 0)) / np.sum(
    1 * l == 0
)
...
def CNN_train(data_folder: Path, opts, seed):
    ...
    s_tr, l_tr, s_val, l_val, s_te, l_te, idx_tr, idx_val, idx_te = tr_val_te_split(
        data, labels, opts, seed
    )

    print(
        "%i samples for train (%0.3f) (%0.3f signal, %0.3f background)"
        % (len(l_tr), len(l_tr) / len(labels), frac(l_tr, 1), frac(l_tr, 0))
    )
    print(
        "%i samples for validation (%0.3f) (%0.3f signal, %0.3f background)"
        % (len(l_val), len(l_val) / len(labels), frac(l_val, 1), frac(l_val, 0))
    )
    print(
        "%i samples for test (%0.3f) (%0.3f signal, %0.3f background)"
    ...
    model.fit(
        ...
        epochs=epochs,
        validation_data=(s_val, to_categorical(l_val)),
        verbose=1,
    )
    ...
    np.savetxt(
        str(data_folder.parent) + "/models/cnn/validation_map", idx_val, fmt="%i"
    )
