...
def SVM_train(data_folder: Path, opts, extra):
    ...
    print(
        "%i samples for train (%0.1f %% of total) (%0.1f signal, %0.1f background)"
        % (
            len(l_tr_svm),
            100 * len(l_tr_svm) / ntot,
            frac(l_tr_svm, 1),
            frac(l_tr_svm, 0),
        )
    )
    print(
        "%i samples for validation (%0.1f %% of total) (%0.1f signal, %0.1f background)"
        % (len(l_val), 100 * len(l_val) / ntot, frac(l_val, 1), frac(l_val, 0))
    )
    print(
        "%i samples for test (%0.1f %% of total) (%0.1f signal, %0.1f background)"
    ...
    if extra:
        print(
            "Using %i features extracted from CNN + Total event energy + Nhits"
            % (s_tr.shape[1])
            - 2
        )
    else:
