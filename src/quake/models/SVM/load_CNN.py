...
def load_cnn(data_folder: Path):
    try:
        ...
        feature_layer = Model(inputs=model.inputs, outputs=model.get_layer("features").output)    

        tr_map = np.loadtxt(str(data_folder.parent)+'/models/cnn/train_map').astype(int)
