...
def get_blob_energies(point_clouds: np.ndarray, setup: dict) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(n):
        ...
        graph = nx.from_numpy_matrix(edges) # Graphs can be visualized with nx.draw(G, with_labels = True)
...
class NPModel:
    ...
    def q_evaluate(self, features: np.ndarray) -> np.ndarray:
        ...
        q_dist: np.ndarray
            q-distribution.
        """
        idx_1 = (((features[:,0] - self.range[0][0]) * self.bins[0]/ self.range[0][1]) ).astype(int)
        idx_2 = (((features[:,1] - self.range[1][0]) * self.bins[1]/ self.range[1][1]) ).astype(int)
...
def neyman_pearson_model(train_features: np.ndarray, train_labels: np.ndarray) -> NPModel:
...
def train_blobs(train_features, train_labels, inference_model) -> Union[NPModel, SVC]:
