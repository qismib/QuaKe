...
from quake.models.attention.attention_dataloading import read_data, Dataset
...
import numpy as np
from quake.models.blob.blob_detection import get_blob_energies, train_blobs
...
def blob_train(data_folder: Path, train_folder: Path, setup: dict):
    ...
    train_features, train_labels = np.array(get_blob_energies(train_generator.inputs, setup)).T, train_generator.targets
    val_features, val_labels = np.array(get_blob_energies(val_generator.inputs, setup)).T, val_generator.targets
    test_features, test_labels = np.array(get_blob_energies(test_generator.inputs, setup)).T, test_generator.targets
