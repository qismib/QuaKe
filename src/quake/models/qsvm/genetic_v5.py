...
def to_quantum(
    ...
    x = ParameterVector("x", length=nb_features + param_shift)
    ...
    for j in range(gates_per_qubits):
        for k in range(nb_qubits):
            ...
            first_feature_idx = genes_unflatted[j, k, 3] + param_shift
            second_feature_idx = genes_unflatted[j, k, 4] + param_shift
...
def fitness_func_wrapper(
    ...
    def fitness_func(ga_instance: pygad.GA, solution: np.ndarray, solution_idx: int) -> Union[np.float64, list[np.float64]]:
        ...
        fitness_value = pd.read_csv(save_path + "/last_generation_fitness_values_" +suffix + ".csv", header=None, index_col=False).to_numpy()
...
def parallel_qpu_run_wrapper(
    ...
    def parallel_qpu_run(ga_instance, offspring_mutation):
        ...
        if ga_instance.keep_elitism != 0:
            ...
            solutions_to_evaluate = np.copy(offspring_mutation)
...
def compute_parallel_kernel_save_data(
    ...
    x_idxss = []
    shuffle_index = list(range(21))
    shuffle(shuffle_index)  # shuffle works in-place
    for k, child in enumerate(genes):
        fmap, x_idxs = to_quantum(
            child, gate_dict, nb_features, gates_per_qubits, nb_qubits, 0, qsvm_connections[shuffle_index[k]], coupling_map = backend_coupling_map
        )
    ...
    if n_gen != 0 or recovery_mode==False:
        for k in range(len(genes)):
            with open(save_path + "/last_generation_genes_mutated" + suffix + ".csv", "w" if k == 0 else "a", encoding="UTF-8") as file:
    ...
    elif recovery_mode and n_gen == 0:
        print("Recovery mode. Fetching the last 17 job results")
        service = QiskitRuntimeService()
        job_results = []
        retrieved_job = service.jobs(backend_name="ibm_nazca", limit = 34)
        for i in range(17):
            print(retrieved_job[-i - 1].job_id())
            job_results.append(retrieved_job[16-i - 17].result())

    del combined_circuits    
    ...
    for k in range(len(genes)):
        ...
        #     clf, param_grid, cv=4, scoring="accuracy", verbose=0)
    ...
    # Saving last-generation information that will be loaded by the fitness function.
        ...
        with open(save_path + "/last_generation_genes_" + suffix + ".csv", "w" if k == 0 else "a", encoding="UTF-8") as file:
            ...
            writer.writerow(genes[k])
    with open(
            save_path + "/rand_indices" + suffix + ".csv", "a", encoding="UTF-8"
        ) as file:
            writer = csv.writer(file)
            writer.writerow(shuffle_index)
