"""
    This module implements functions for generating various quantum featuremaps.
"""
    ...
    for _ in range(repeats):
        for q in range(qubits):
            if q % 2 == 0:
                ...
                custom2_featuremap.rz(np.arccos(0.4 * x[q] * x[q]), q)
...
def genetic_attention_highres(
    x: ParameterVector, repeats: int = 1, align: bool = False
) -> QuantumCircuit:
    """
    Entangled featuremap generated by a genetic optimization on attention features.
    ...
    genetic_attention: QuantumCircuit
        The quantum featuremap.
    """
    genetic_attention = QuantumCircuit(2)
    if align:
        theta = ParameterVector("θ", 2)
        genetic_attention.rx(theta[0], 0)
        genetic_attention.rx(theta[1], 1)
    for _ in range(repeats):
        genetic_attention.rx(4 * x[0] * x[1], 0)
        genetic_attention.cx(0, 1)
        genetic_attention.ry(np.arcsin(2 / np.pi * x[1]), 0)
        genetic_attention.rx(2 * x[0], 1)
    return genetic_attention
